...
public class ReusableMethods {
	...
	public boolean SortOfWebElementsInDecOrderWtoutSplit(String AppElement) throws Exception
	{
		boolean flag=false;
		//First we need to initialize an List to store getText of all the elements as integer
		List<String> obtainedList = new ArrayList<>(); 
	...
	public String getCurrentDate()
	{
		...
		return dateFormat1.format(cal.getTime()).toString(); 
    }
	public String getCurrentMonthNYear()
	{
	...
	public String getYesturdayDate()
	{
	...
	public String getYesterdayInYYYYFormat()
	{
...
public String getCurrentTime()
{
	...
	return(dfor.format(obj.getTime()));
}

public boolean listOfDates(String AppElement, String CurrentMonth, ExtentTest logInfo) throws Exception
{
	boolean flag=false;
    List<String> obtainedList = new ArrayList<>(); 
    List<WebElement> elementList = FindElements(AppElement);
	for(WebElement i:elementList){
		obtainedList.add(i.getText().trim());
	}
	for(int i=0; i<elementList.size(); i++)
	{
		
		if(obtainedList.get(i).contains(CurrentMonth))
		{
			logInfo.pass("All the dates are from current month");
		}
		else {
			logInfo.fail("All the dates are not from current month");
		}
	}
	ArrayList<String> sortedList = new ArrayList<>();  
	for(String j:obtainedList){
